...
import com.chainmaker.jobservice.core.antlr4.sql.SqlBaseBaseVisitor;
import com.chainmaker.jobservice.core.antlr4.sql.SqlBaseLexer;
import com.chainmaker.jobservice.core.antlr4.sql.SqlBaseParser;
...
public class LogicalPlanBuilder extends SqlBaseBaseVisitor<Node> {
    ...
    public LogicalPlanBuilder(String sqlText) {
    ...
    /***
     * @description 解析器入口规则，暂只支持query语句，不支持create、drop等
     * @param context
     * @return com.chainmaker.jobservice.core.parser.plans.LogicalPlan
     * @author gaokang
    ...
    public LogicalPlan visitQuery(SqlBaseParser.QueryContext context) {
        if (context.queryNoWith() instanceof SqlBaseParser.SingleInsertQueryContext) {
            return visitSingleInsertQuery((SqlBaseParser.SingleInsertQueryContext) context.queryNoWith());
        } else {
            throw new ParserException(DEFAULT_ERROR + ": " + context.queryNoWith().getChild(0).getText());
        }
    }

    /***
     * @description 判断是否有ORDER BY等，如果有则生成Sort节点
     * @param context
     * @return com.chainmaker.jobservice.core.parser.plans.LogicalPlan
     * @author gaokang
     ...
     */
    @Override
    public LogicalPlan visitSingleInsertQuery(SqlBaseParser.SingleInsertQueryContext context) {
        if (context.queryTerm() instanceof SqlBaseParser.QueryTermDefaultContext) {
            if(context.queryOrganization().getChildCount() == 0) {
                return visitQueryTermDefault((SqlBaseParser.QueryTermDefaultContext) context.queryTerm());
            } else {
                LogicalPlan sortChild = visitQueryTermDefault((SqlBaseParser.QueryTermDefaultContext) context.queryTerm());
                Expression expression = visitQueryOrganization(context.queryOrganization());
                return new LogicalSort(expression, sortChild);
            }
        } else {
            throw new ParserException(DEFAULT_ERROR + ": " + context.queryTerm().getText());
        }
    }
    ...
    public LogicalPlan visitQueryTermDefault(SqlBaseParser.QueryTermDefaultContext context) {
        if (context.queryPrimary() instanceof SqlBaseParser.QueryPrimaryDefaultContext) {
            return visitQuerySpecification(((SqlBaseParser.QueryPrimaryDefaultContext) context.queryPrimary()).querySpecification());
        } else {
    ...
    /***
     * @description 暂不支持ORDER BY等
     * @param context
     * @return com.chainmaker.jobservice.core.parser.tree.Expression
     * @author gaokang
     * @date 2022/8/14 15:58
     */
    @Override
    public Expression visitQueryOrganization(SqlBaseParser.QueryOrganizationContext context) {
        throw new ParserException(DEFAULT_ERROR + ": " + context.getChild(0).getText());

    }

    /***
     * @description 根据类型区分联邦查询（软硬件）和联邦学习
     * @param context
     * @return com.chainmaker.jobservice.core.parser.plans.LogicalPlan
     * @author gaokang
     ...
     */
    @Override
    public LogicalPlan visitQuerySpecification(SqlBaseParser.QuerySpecificationContext context) {
        if (context.kind.getText().equals("SELECT")) {
            ...
            if (context.namedExpressionSeq() instanceof SqlBaseParser.FederatedLearningExpressionContext) {
                FederatedLearningExpression expression = visitFederatedLearningExpression((SqlBaseParser.FederatedLearningExpressionContext) context.namedExpressionSeq());
                return new FederatedLearning(expression, children);
            } else {
                FaderatedQueryExpression projectList = visitFederatedQueryExpression((SqlBaseParser.FederatedQueryExpressionContext) context.namedExpressionSeq());
                return new LogicalProject(projectList, children);
            }

        } else {
            throw new ParserException(DEFAULT_ERROR + ": " + context.kind.getText());
        }
    }

    /***
     * @description 生成Filter节点
     * @param context
     * @return com.chainmaker.jobservice.core.parser.plans.LogicalPlan
     * @author gaokang
     * @date 2022/8/19 13:38
     */
    public LogicalPlan visitWhere(SqlBaseParser.QuerySpecificationContext context) {
        Expression expression = visitBooleanExpression(context.where);
        List<LogicalPlan> children = visitFrom(context.fromClause());
        return new LogicalFilter(expression, children);
    }

    /***
     * @description 处理booleanExpression
     * @param context
     * @return com.chainmaker.jobservice.core.parser.tree.Expression
     * @author gaokang
    ...
    public Expression visitBooleanExpression(SqlBaseParser.BooleanExpressionContext context) {
        ...
        } else if (context instanceof SqlBaseParser.BooleanDefaultContext) {
            return visitBooleanDefault((SqlBaseParser.BooleanDefaultContext) context);
        } else {
    ...
    @Override
    public Expression visitBooleanDefault(SqlBaseParser.BooleanDefaultContext context) {
        return visitPredicated(context.predicated());
    }
    ...
    /***
     * @description 处理primaryExpression，目前只支持一部分
     * @param context
     * @return com.chainmaker.jobservice.core.parser.tree.Expression
     * @author gaokang
    ...
    public FunctionCallExpression visitFunctionCall(SqlBaseParser.FunctionCallContext context) {
        String function = context.qualifiedName().getChild(0).getText();
    ...
    /***
     * @description FROM可能有多个表节点,通过visitFrom返回
     * @param context
     * @return com.chainmaker.jobservice.core.parser.plans.LogicalPlan
     * @author gaokang
    ...
    public LogicalPlan visitRelation(SqlBaseParser.RelationContext context) {
        if (context.joinRelation().size() == 0) {
            return visitRelationPrimary(context.relationPrimary());
        } else {
            if (context.joinRelation(0).NATURAL() == null && context.joinRelation(0).joinCriteria() != null) {
                LogicalJoin.Type joinType;
                if (context.joinRelation(0).joinType().LEFT() != null) {
                    joinType = LogicalJoin.Type.LEFT;
                } else if (context.joinRelation(0).joinType().RIGHT() != null) {
                    joinType = LogicalJoin.Type.RIGHT;
                } else if (context.joinRelation(0).joinType().FULL() != null) {
                ...
                LogicalPlan right = visitRelationPrimary(context.joinRelation(0).relationPrimary());
                Expression expression = visitBooleanDefault((SqlBaseParser.BooleanDefaultContext) context.joinRelation(0).joinCriteria().booleanExpression());
    ...
    public LogicalPlan visitRelationPrimary(SqlBaseParser.RelationPrimaryContext context) {
        if (context instanceof SqlBaseParser.TableNameContext) {
            ...
            if (((SqlBaseParser.AliasedQueryContext) context).queryNoWith() instanceof SqlBaseParser.SingleInsertQueryContext) {
                return visitSingleInsertQuery((SqlBaseParser.SingleInsertQueryContext) ((SqlBaseParser.AliasedQueryContext) context).queryNoWith());
            } else {
                throw new ParserException(DEFAULT_ERROR + ": " + context.getText());
            }
        } else if (context instanceof SqlBaseParser.AliasedRelationContext) {
            if (((SqlBaseParser.AliasedRelationContext) context).relation().joinRelation().size() == 0) {
                throw new ParserException(DEFAULT_ERROR + ": " + context.getText());
            } else {
                LogicalJoin logicalJoin = (LogicalJoin) visitRelation(((SqlBaseParser.AliasedRelationContext) context).relation());
                if (((SqlBaseParser.AliasedRelationContext) context).strictIdentifier() != null) {
                    logicalJoin.setAlias(((SqlBaseParser.AliasedRelationContext) context).strictIdentifier().getText());
                }
                return logicalJoin;
            }
        } else {
    ...
    public LogicalPlan visitTableName(SqlBaseParser.TableNameContext context) {
        ...
        if (context.strictIdentifier() != null) {
            String alias = context.strictIdentifier().getText();
...
//    }
    ...
    public NamedExpression visitNamedExpression(SqlBaseParser.NamedExpressionContext context) {
        if (context.identifierList() == null) {
            if (context.identifier() == null) {
                ...
                Identifier identifier = visitIdentifier(context.identifier());
                Expression expression = visitExpression(context.expression());
    ...
    public FederatedLearningExpression visitFederatedLearningExpression(SqlBaseParser.FederatedLearningExpressionContext context) {
        ...
        Identifier value = new Identifier(context.flModelSeq(0).flModel(0).getText());
