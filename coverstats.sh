...
# 定义默认注释率和单元测试率
default_comment_rate=15
default_unit_test_rate=20
...
if [[ -n "$comment" && -n "$code" ]]; then
    ...
    # 判断是否达到注释率标准
    if (( $(echo "$percentage > $default_comment_rate" | bc -l) )); then
        echo "The comment rate has reached the standard: $default_comment_rate%."
    else
        echo "Comment rate did not meet the standard: $default_comment_rate%."
    fi
else
